<?php
	...
	if (!empty($_POST)) {
		...
		switch ($_POST['action']) {
			...
			case 'update':
				$pretty_cmd	= 'update';
				$issue_cmd	= "sudo -u www-data sudo service {$_INICONF['servicename']} stop && sudo -u steam {$_INICONF['steamcmdbin']} +login anonymous +force_install_dir {$_INICONF['force_install_dir']} +app_update {$_INICONF['steamappid']} +quit | tee /tmp/update.log"; // | tee /tmp/update.log
				$logwatch	= true;
			...
			case 'reinstall':
				...
				$issue_cmd	= "sudo -u www-data sudo service {$_INICONF['servicename']} stop && sudo rm -rf /home/steam/ark_ds/* && sudo -u steam {$_INICONF['steamcmdbin']} +login anonymous +force_install_dir {$_INICONF['force_install_dir']} +app_update {$_INICONF['steamappid']} +quit | tee /tmp/update.log"; // | tee /tmp/update.log
